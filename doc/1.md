...
AOP的三个部分: 目标类, 通知类, 通知和切点的织入(切面)

都需要被Spring容器管理, 以下是步骤:

1.   目标类和通知类

```java
@Component("target")
public class TargetImpl implements UserService {  // 在com.zigzag.ch1.aop包下
  @Override
  public void show1() {
...
@Component
@Aspect  // 通知类添加注解,指明这时一个通知类
public class AnnoAdvice {

  @Around("execution(public void com.zigzag.ch1.aop.TargetImpl.*(..))")
...
```xml
<!--配置组件注解解析-->
<context:component-scan base-package="com.zigzag.ch1.aop" />
...
![image-20240520191151313](./assets/1/image-20240520191151313.png)
...
public class AnnoAdvice {
  ...
  // 抽取切面表达式
  @Pointcut("execution(public void com.zigzag.ch1.aop.TargetImpl.*(..))")
  ...
  @Before("pointCut()")  // 直接使用抽取的切面
  public void before() {
    System.out.println("前置");
  }

  @AfterReturning(value = "execution(public String com.zigzag.ch1.aop.TargetImpl.*(..))", returning = "result")  // 指定返回值
  ...
  @AfterThrowing(value = "execution(public void com.zigzag.ch1.aop.TargetImpl.*(..))", throwing = "th")  // 指定异常
...
在xml方式配置AOP时没接住`<aop:config>`标签,最终加载`AspectJAwareAdvisorAutoProxyCreator`执行实现自`BeanPostProcessor`的方法,完成代理对象的创建

而在注解方式中, 添加了`<aop:aspectj-autoproxy/>`;

该注解是由`new AspectJAutoProxyBeanDefinitionParser()`的parse方法处理;

该方法执行了`AopNamespaceUtils.registerAspectJAnnotationAutoProxyCreatorIfNecessary(parserContext, element);`  是和xml方式一样的处理方式,注册了

**配置类注解方式:**

`@EnableAspectJAutoProxy`

使用`@Import`导入相关解析类,`@Import(AspectJAutoProxyRegistrar.class)`, 同样还是执行了`AopConfigUtils.registerAspectJAnnotationAutoProxyCreatorIfNecessary(registry);`,注册了
...
![image-20240520195008041](./assets/1/image-20240520195008041.png)
